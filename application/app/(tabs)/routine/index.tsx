...
import { Archive, ChevronLeft, ChevronRight, ClipboardList, Plus } from "lucide-react-native";
...
export default function RoutineScreen() {
  ...
  const archivedRoutines = useMemo(() => routines.filter((r) => !r.active), [routines]);
      ...
      >
        {loading ? (
          ...
          <View className="items-center rounded-2xl bg-card p-8">
            ...
            <Pressable onPress={fetchRoutines} className="rounded-full bg-white/10 px-5 py-2.5">
            ...
            {displayedRoutines.map((routine) => (
              ...
              >
                <View className="mb-2 flex-row items-center justify-between">
                  <Text className="flex-1 text-lg font-semibold text-white">{routine.title}</Text>
                  {routine.active && (
                ...
                {routine.description ? (
                  <Text className="mb-2 text-sm text-white/50" numberOfLines={2}>
                    {routine.description}
                  </Text>
                ) : null}
                <Text className="text-xs text-white/30">{formatDate(routine.createdAt)}</Text>
              </Pressable>
            ))}
