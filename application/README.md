...
- "+" 버튼 → `/(tabs)/routine/new`로 이동 (루틴 생성)
- `useFocusEffect`로 화면 복귀 시 목록 자동 갱신
...
#### 루틴 생성 — `app/(tabs)/routine/new.tsx`

루틴을 새로 만드는 화면. 이름/설명 입력 + 운동 종목 구성.

**레이아웃:**

- 헤더: ← 뒤로가기 | "루틴 만들기" | "저장" 버튼
- 루틴 이름 입력 (필수), 설명 입력 (선택)
- 구성 종목 리스트 (운동 카드)
- "+ 종목 추가" 버튼
...
- 순서 번호 표시
- 세트 수 조절: ±1, 범위 1~20
- 휴식 시간 조절: ±30초, 범위 0~600초, "N분 N초" 포맷
- 순서 변경 (↑↓ 버튼)
- 삭제 (🗑 버튼)

**저장 조건:** 제목이 입력되고 운동이 1개 이상일 때 저장 버튼 활성화

**데이터 플로우:**

```
"종목 추가" → select-exercises 화면 이동
  → 선택 완료 후 복귀 → useFocusEffect에서 exercise-selection 저장소 읽기
  → 선택된 운동을 items에 추가 (기본값: 4세트, 90초 휴식)
"저장" → routineApi.createRoutine() → 성공 시 목록으로 복귀
```

#### 운동 종목 선택 — `app/(tabs)/routine/select-exercises.tsx`

루틴에 추가할 운동을 다중 선택하는 화면.

**레이아웃:**

- 헤더: ← 뒤로가기 | "종목 선택" | "확인 (N)" 버튼
- 검색 입력 (운동 이름)
- 부위 필터 칩 (가로 스크롤): 전체, 가슴, 등, 어깨, 하체, 팔, 코어, 유산소, 기타
- 운동 목록 (FlatList, 체크박스 다중 선택)

**동작:**

- `exerciseApi.getExercises()`로 전체 운동 목록 로드
- 클라이언트 사이드 필터: 부위 + 검색어
- "확인" → `setSelectedExercises()`로 임시 저장소에 저장 → `router.back()`

**화면 간 데이터 전달:**
`lib/store/exercise-selection.ts`의 모듈 레벨 변수 사용. `set` 후 `get`하면 자동 초기화되어 1회성 전달에 적합.
...
### 6.2 format.ts — 날짜/시간 포맷 유틸

| 함수                             | 포맷                                    | 용도                |
| -------------------------------- | --------------------------------------- | ------------------- |
| `formatDate(dateString)`         | "2024년 3월 15일"                       | 루틴 목록 생성일 등 |
| `formatRelativeDate(dateString)` | "오늘" / "어제" / "3일 전" / "3월 15일" | 운동 기록 목록      |
| `formatRestTime(seconds)`        | "1분 30초" / "0초" / "2분"              | 루틴 휴식 시간 표시 |
| `formatElapsedTime(seconds)`     | "01:23:45"                              | 운동 세션 타이머    |

### 6.3 utils.ts — cn() 유틸리티
...
### 6.6 api/exercise.ts — 운동 종목 API

| 함수                         | 메서드 | 경로             | 동작                |
| ---------------------------- | ------ | ---------------- | ------------------- |
| `exerciseApi.getExercises()` | GET    | `/api/exercises` | 전체 운동 종목 조회 |

### 6.7 api/routine.ts — 루틴 API

| 함수                              | 메서드 | 경로                | 동작                       |
| --------------------------------- | ------ | ------------------- | -------------------------- |
| `routineApi.getRoutines()`        | GET    | `/api/routines`     | 루틴 목록 조회             |
| `routineApi.getRoutineDetail(id)` | GET    | `/api/routines/:id` | 루틴 상세 (운동 구성 포함) |
| `routineApi.createRoutine(req)`   | POST   | `/api/routines`     | 루틴 생성                  |
| `routineApi.deleteRoutine(id)`    | DELETE | `/api/routines/:id` | 루틴 삭제                  |

### 6.8 api/workout.ts — 운동 세션 API
...
### 6.9 store/exercise-selection.ts — 운동 선택 임시 저장소

화면 간 운동 선택 데이터 전달을 위한 모듈 레벨 저장소.

| 함수                              | 동작                                              |
| --------------------------------- | ------------------------------------------------- |
| `setSelectedExercises(exercises)` | 선택된 운동 목록 저장                             |
| `getSelectedExercises()`          | 저장된 운동 목록 반환 후 즉시 초기화 (1회성 읽기) |

Context나 AsyncStorage보다 가볍고, 같은 프로세스 내 페이지 간 전달에 적합. `select-exercises` → `new` 화면 간 데이터 전달에 사용.

### 6.10 types/auth.ts — 인증 타입
...
### 6.11 types/exercise.ts — 운동 종목 타입

| 타입/상수          | 용도                                                                                                |
| ------------------ | --------------------------------------------------------------------------------------------------- |
| `BodyPart`         | 운동 부위 유니온 타입 (`CHEST \| SHOULDER \| BACK \| ARM \| LOWER_BODY \| CORE \| CARDIO \| OTHER`) |
| `BODY_PART_LABEL`  | BodyPart → 한글 매핑 (`CHEST` → "가슴" 등)                                                          |
| `BODY_PARTS`       | 부위 목록 배열 (UI 순서대로)                                                                        |
| `ExerciseResponse` | 운동 종목 응답 (id, name, bodyPart, partDetail, custom)                                             |

### 6.12 types/routine.ts — 루틴 타입
...
| `RoutineItemCreateRequest` | 루틴 운동 생성 요청 (exerciseId, sets, restSeconds)                                |
| `RoutineCreateRequest`     | 루틴 생성 요청 (title, description, routineItems[])                                |

### 6.13 types/workout.ts — 운동 세션 타입
...
```
루틴 탭 진입 (useFocusEffect로 매번 갱신)
  └─ routineApi.getRoutines() → 목록 표시
       ├─ "+" 버튼 → 루틴 생성 화면 (routine/new)
...
루틴 생성 (routine/new):
  ├─ 이름/설명 입력
  ├─ "종목 추가" → 운동 선택 화면 (routine/select-exercises)
  │    ├─ exerciseApi.getExercises() → 운동 목록 로드
  │    ├─ 부위 필터 + 검색으로 필터링
  │    ├─ 다중 선택 후 "확인" → exercise-selection 저장소에 저장 → 복귀
  │    └─ useFocusEffect에서 저장소 읽기 → items에 추가
  ├─ 세트 수/휴식 시간 ± 조절
  ├─ 운동 순서 변경 (↑↓) / 삭제
  └─ "저장" → routineApi.createRoutine() → 루틴 목록으로 복귀
```
...
| 홈 탭                 | 플레이스홀더                            |
| 커뮤니티 탭           | 플레이스홀더                            |
| 루틴 수정             | 루틴 생성은 구현됨, 수정(편집)은 미구현 |
| 자유 운동 (루틴 없이) | 미구현                                  |
| 커스텀 운동 종목 추가 | 미구현 (백엔드 API 확인 필요)           |
